...
- `E2eeAccountTrustManifestModule` 集中创建、解析、ARK 锚验证、恢复 genesis 锚验证、分页 history 链验证、服务端投影核对和恢复 ARK 绑定，只输出不可外部构造的 `E2eeVerifiedMembership`。
...
- 恢复文件必须保存 genesis 完整 manifest、digest 和 epoch1 trust public key 锚；history 按 generation 无跳跃验证。单批最多 256 份，可分页继续。单一恶意服务器仍可回放有效旧前缀造成回滚或拒绝服务，但不能伪造 ARK 或新 epoch 链。
