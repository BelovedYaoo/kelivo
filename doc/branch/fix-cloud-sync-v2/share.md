...
- 组合验证：`flutter analyze lib test` 通过；Store、Coordinator、Journal、传输协议、配置域与冲突展示共 77 项测试通过；聊天域相关 63 项测试通过。完整全仓分析仍会命中既有 `dependencies/mcp_client` 测试缺少 `package:test` 的 736 条错误。
