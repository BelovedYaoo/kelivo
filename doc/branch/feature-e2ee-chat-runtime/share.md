...
- 并行中：匿名 LocalVault 的 Windows 根路径边界、Windows 安全槽 reparse 边界、附件上传取消/硬截止与附件事务一致性正在独立分支修复，未合并前不得视为闭环。
- 下一步：合并并复验上述安全修复，完成客户端 data-rekey v2 执行器，再接入移动端恢复二维码/文件 UI、服务端恢复挑战和后台调度可信世代预检。
