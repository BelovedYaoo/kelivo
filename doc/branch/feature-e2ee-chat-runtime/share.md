...
- 当前：生产聊天内容运行时、配置 Provider、附件上传断点状态、schema 17 与附件 HTTP 已接入；消息密文 payload 已硬切 v2 完整附件身份，仍需完成附件文件传输/领域闭环。
...
- 下一步：硬切 schema 18 持久下载与分块摘要状态，接通原子密文文件落盘、上传协调器、下载校验和聊天适配器。
