...
- 进行中 #111：第一条纵向链路先接助手头像/背景；配置 payload v4、SQLCipher schema 26、配置资产命令与资产 GC 已完成，配置资产复用附件 R2 密文通道。
- 已完成 #111 远端半链：pull 预取会识别助手头像/背景身份，下载认证后在同一事务登记配置资产并水合 Provider；配置-only 拉取已纳入提交后刷新与回收。下一步接本地选择后的内容寻址物化、上传草稿与动态快照。
