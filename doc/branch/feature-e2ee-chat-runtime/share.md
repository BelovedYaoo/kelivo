...
- 已完成：#110 将用户导出的数据库恢复为便携明文 SQLite；精确识别并桥接官方最小 schema 12 与旧附件扩展，导入时保留当前 E2EE 控制面、生成覆盖墓碑/outbox，并重定位、校验旧附件路径与正文标记。全部附件消息、上传草稿和 outbox 在同一事务提交，失败时聊天与缓存均回滚；真实旧备份曾验证 52 个会话、265 条消息，相关 101 项回归通过、1 项 Windows 权限跳过。
- 已确认：并非所有多媒体都无法同步，聊天结构化图片/文件附件的数据面可用；助手头像、助手背景、用户头像、供应商头像及正文/工具事件中的本地媒体缺少通用受管资产数据面，由 #111 跟踪；远程内联媒体 marker 阻塞 E2EE outbox 由 #114 跟踪；备份合并同步与跨存储持久补偿分别由 #112、#113 跟踪。
...
- 进行中 #111：配置 payload v4、SQLCipher schema 26、配置资产命令与资产 GC 已完成，配置资产复用附件 R2 密文通道。
- 已完成 #111 配置媒体纵向链路：助手头像/背景、用户头像与供应商头像均支持本地内容寻址物化、上传中跨重启恢复、密文上传提交后动态快照、远端下载水合、清空和删除；助手额外覆盖复制。本机路径不进入 payload，配置-only 拉取纳入提交后刷新与回收。
- 下一步 #111：将助手生成图片与 MCP 本地图片转换为结构化受管资产引用；字体是否纳入同步仍需产品决定。
- 已完成 #111 助手生成图片终态链路：正常完成、失败和中断均在最终检查点把绝对本机 Markdown 图片转换为结构化附件，附件上传草稿与消息出站原子提交，并向界面发布实际持久化快照；助手复用现有附件预览。失效文件不落库，漏网本机 Markdown 路径在同步适配器继续失败关闭。
- 已完成 #111 MCP 图片纵向链路：终态工具本地图片与助手正文图片统一物化为消息受管附件，工具事件只保存加密的附件序号；同步入站/出站严格校验图片类型与序号，工具卡片、导出和后续模型上下文按当前设备附件路径水合。URL-only MCP 图片不再误作 `null` base64，图片落盘失败不再静默丢失。
- 已完成 #111 旧数据库备份兼容：导入阶段会把旧 MCP 图片 marker 重定位到当前工作区、合并进同一消息附件序列并转换为序号；正常覆盖事务继续原子创建附件草稿与聊天 outbox。
...
- 已完成（2026-08-08 三项大更改全部移植，main@16c2d655）：
  - cbac3dc3（ee4dfbd0）：提供商 reasoning 签名保留。openai_common 新增 _ReasoningDetailsAccumulator（OpenRouter delta 拼接 / 快照检测），Claude upstream 剥离无签名 reasoning 回显（否则 Anthropic 拒绝 thinking.signature），工具消息与完成点无条件携带 reasoningDetails；google_common/gemini 首个 functionCall part 缺 thoughtSignature 回填占位符（Gemini 3 整请求校验失败），ToolCallInfo metadata 携带签名精确重放；chat_actions/message_builder 持久化并在后续请求回显；ChatStreamChunk 新增 reasoningDetails 字段。新移植 openai_claude_proxy_thinking_test 406 行，50/50 通过。
...
- 已完成（2026-08-08 死代码清理与 #115 方向修正，main@…）：删除无生产调用方的快照恢复死链（restoreDatabaseSnapshot/replaceBackupSnapshot/_importBackupSnapshot，净删 204 行）；**修正 #115 方向**——mergeDatabaseSnapshot 恢复生成同步意图批次（chat_service『快照冲突合并』测试证实 #112 意图=merge 补入会话应推送云端；此前依 data_sync 测试误判 isEmpty 是错误方向）；data_sync merge 断言改 isNotEmpty、snapshot_test 删 2 个死实现用例、temporary_conversation 2 处改生产方法。108/108 通过。cloud_sync content gate 24 失败经 stash 复验为环境安全槽 internalState（#81 残留，非本次改动；隔离入口同样失败，待槽清理后复验）。
- 已完成（2026-08-08 遗留项复核与收尾，main@待提交）：scout 并行复核全部遗留 issue，多数已实现（旧记录过时）：
  - #51 成员清单 v2：Dart+Rust 两侧均已 v2（header 260B/offset 224 operationAuthorizationDigest/memberCount@256），op3 自撤销 intentDigest 绑定 + 跨语言固定向量测试锁定——无需改动，发布门禁的两端对齐已达成。
  - #49 移动恢复 UI：导出（mobile_recovery_media_export_page）+ 导入（mobile_account_recovery_page）+ Native FFI 装配（e2ee_account_authenticator/main.dart）全链已接通；本次补 AndroidManifest CAMERA 权限 + uses-feature 显式声明（mobile_scanner 依赖库合并，软点收敛）。
  - #84/#85：持久明文日志退役与受管根句柄删除完整链路已落地（含 Android /data/user/0 bind mount 别名验证、openat 避 SIGSYS、SharedPreferences 删除证明 ABI20）；恢复介质页两处 developer.log 复核为静态事件（固定 name/level/无变量），符合惯例无需改。
  - #112：全量导入同步意图链路闭环（runImportBatch/importPortableChats 同事务写 intent+outbox + _recoverStartup 崩溃恢复），merge 已按 #115 不生成——关闭；restoreDatabaseSnapshot/replaceBackupSnapshot 死代码（无生产调用方）记录为可选清理。
  - #113：跨存储持久补偿的原始场景已被「导入同事务 + outbox 跨重启恢复」覆盖——建议关闭或改记为已覆盖。
  - #116：已修——DioHttpClient 支持注入 HttpClientAdapter，网络失败测试改用 _FailingDioAdapter 确定性失败（替代 bind/close 端口方案，消除端口复用竞态）；handshake 测试保持真实网络（testWidgets 会 mock 400 不适用），单跑稳定、并行偶发挂起属环境级。
