...
void main() {
  ...
  Future<GenerationBeginResult> beginFirstSend() =>
      repository.beginSendGeneration(
        conversation: conversation,
        userMessage: user('user-1', turnId: 'turn-1'),
        assistantMessage: assistant('assistant-1', turnId: 'turn-1'),
        runId: 'run-1',
      );
  ...
  Future<Conversation> appendFuture(Conversation current) async {
    final afterUser = await repository.appendLinearMessageToConversation(
      ...
      message: user('user-2', turnId: 'turn-2'),
    );
  ...
  test('run insert failure rolls back every linear send row', () async {
    ...
    await expectLater(
      repository.beginSendGeneration(
        ...
        userMessage: user('user-2', turnId: 'turn-2'),
        assistantMessage: assistant('assistant-2', turnId: 'turn-2'),
        runId: 'run-1',
      ),
      throwsA(anything),
    );
  ...
  test('invalid begin input is rejected before any database write', () async {
    expect(
      () => repository.beginSendGeneration(
        conversation: conversation,
        userMessage: assistant('not-user', turnId: 'turn-invalid'),
        assistantMessage: assistant('assistant-1', turnId: 'turn-invalid'),
        runId: 'run-1',
      ),
      throwsArgumentError,
    );
