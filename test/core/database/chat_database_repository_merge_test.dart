...
void main() {
  group('ChatDatabaseRepository merge snapshot', () {
    ...
    test('同 conversation ID 异内容时整会话 remap 并可重复去重', () async {
      ...
      final remappedMessage = (await live.getMessagesRange(
        remappedId,
        start: 0,
        limit: 1,
      )).single;
      expect(remappedMessage.groupId, startsWith('merge-'));
      expect(remappedMessage.turnId, startsWith('merge-'));
      expect(
        (await live.getTurnCreatedAts(remappedId)).keys,
        contains(remappedMessage.turnId),
      );
      expect(await live.getToolEvents(remappedMessage.id), const [
        {'id': 'tool', 'content': 'imported'},
      ]);
      expect(
        await live.getGeminiThoughtSignature(remappedMessage.id),
        'sig-imported',
      );
