  ...
  group('RestoreBusinessLease', () {
    ...
    test('fails immediately while another process owns the lease', () async {
      ...
      final helperRoot = await helperParent.createTemp(
        'kelivo_restore_business_lease_helper_',
      );
      addTearDown(() async {
      ...
      final helper = File(
        p.join(helperRoot.path, 'business_lease_helper.dart'),
      );
      ...
      Future<String> waitForReady() async {
        ...
        throw StateError('helper exited with $code: ${await stderr}');
      }

      final ready = await waitForReady().timeout(const Duration(seconds: 15));
      expect(ready, 'ready');
