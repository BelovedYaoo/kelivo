...
class ChatDatabaseRepository {
    ...
    try {
      ...
      try {
        database.execute(
          ...
          "generation_status = 'interrupted' WHERE is_streaming != 0;",
        );
  ...
  Future<AssetGcLeaseAttempt> tryAcquireAssetGcLease({
    ...
    final expiresAt = acquiredAt.add(leaseDuration);
    return _db.transaction(() async {
      await _db.customStatement(
        '''
        INSERT INTO asset_gc_lease_rows(lease_name, owner_token, expires_at)
        VALUES ('asset-gc', ?, ?)
        ON CONFLICT(lease_name) DO UPDATE SET
          owner_token = excluded.owner_token,
          expires_at = excluded.expires_at
        WHERE asset_gc_lease_rows.owner_token = excluded.owner_token
          OR asset_gc_lease_rows.expires_at <= ?;
        ''',
        [
          cleanOwnerToken,
      ...
      final row = await _db.customSelect('''
            ...
            FROM asset_gc_lease_rows
            WHERE lease_name = 'asset-gc'
            LIMIT 1;
            ''').getSingle();
      return AssetGcLeaseAttempt(
        acquired: row.read<String>('owner_token') == cleanOwnerToken,
  ...
  Future<bool> releaseAssetGcLease({required String ownerToken}) async {
    ...
    return _db.transaction(() async {
      await _db.customStatement(
        '''
        DELETE FROM asset_gc_lease_rows
        WHERE lease_name = 'asset-gc' AND owner_token = ?;
        ''',
        [cleanOwnerToken],
      );
      return (await _db
  ...
  Future<Conversation?> getConversation(
    ...
    return _observer.measure(
      ...
      () async {
        ...
        return _conversationFromRow(row, includeMessageIds: includeMessageIds);
      },
  ...
  Future<List<ChatMessage>> getMessagesForTurn(String turnId) async {
    final cleanTurnId = turnId.trim();
    if (cleanTurnId.isEmpty) return const <ChatMessage>[];
    final rows =
        await (_db.select(_db.messageRows)
              ..where((row) => row.turnId.equals(cleanTurnId))
              ..orderBy([(row) => OrderingTerm.asc(row.messageOrder)]))
            .get();
    return _messagesFromRowsWithParts(rows);
  }

  Future<String?> getConversationIdForTurn(String turnId) async {
    final cleanTurnId = turnId.trim();
    if (cleanTurnId.isEmpty) return null;
    final persisted =
        await (_db.select(_db.turnRows)
              ..where((row) => row.id.equals(cleanTurnId))
              ..limit(1))
            .getSingleOrNull();
    if (persisted != null) return persisted.conversationId;
    final message =
        await (_db.select(_db.messageRows)
              ..where((row) => row.turnId.equals(cleanTurnId))
              ..limit(1))
...
FROM conversation_rows c, json_each(c.version_selections_json) selection
WHERE selection.key = ?
LIMIT 1;
''',
          variables: [Variable<String>(cleanGroupId)],
          readsFrom: {_db.conversationRows},
        )
        .getSingleOrNull();
    return row?.read<String>('conversation_id');
  }

  Future<Map<String, DateTime>> getTurnCreatedAts(String conversationId) async {
  ...
  /// 加载模型上下文所需的已选线性消息版本。
  ///
  /// 版本折叠、截断索引应用、尾部限制和消息部件组装有意在一条 SQL 语句中完成，
  /// 避免仅为丢弃前缀而将大型会话完整载入内存。
  ...
  Future<bool> replaceMessageAssetReferences({
    required String conversationId,
    required String revisionId,
            ...
            WHERE g.not_before <= ?
              ...
              AND NOT EXISTS (
                ...
                WHERE instr(
                  ...
                  OR (
                    a.thumbnail_path IS NOT NULL
                    AND (
                      instr(
                        lower(m.content),
                        lower('[image:' || a.thumbnail_path || ']')
                      ) > 0
                      OR instr(
                        lower(m.content),
                        lower('[file:' || a.thumbnail_path || '|')
                      ) > 0
                    )
                  )
              )
              AND NOT EXISTS (
                ...
                WHERE other.id <> a.id
                  ...
                  AND (
                    ...
                    OR (
                      a.thumbnail_path IS NOT NULL
                      AND other.path = a.thumbnail_path COLLATE NOCASE
                    )
                    OR (
                      other.thumbnail_path IS NOT NULL
                      AND other.thumbnail_path = a.path COLLATE NOCASE
                    )
                    OR (
                      a.thumbnail_path IS NOT NULL
                      AND other.thumbnail_path = a.thumbnail_path
                        COLLATE NOCASE
                    )
                  )
              )
              AND NOT EXISTS (
                SELECT 1
                FROM asset_rows other
                LEFT JOIN asset_gc_rows other_gc
                  ON other_gc.asset_id = other.id
                WHERE other.id <> a.id
                  AND (
                    other.path = a.path COLLATE NOCASE
                    OR (
                  ...
                  AND (
                    other_gc.asset_id IS NULL
                    OR other_gc.not_before > ?
                  )
              )
  ...
  Future<bool> isAssetGcClaimStillValid(
    ...
    final row = await _db
            ...
            AND NOT EXISTS (
              ...
              WHERE other.id <> a.id
                ...
                AND (
                  ...
                  OR (
                    a.thumbnail_path IS NOT NULL
                    AND other.thumbnail_path = a.thumbnail_path COLLATE NOCASE
                  )
                )
            )
            AND NOT EXISTS (
        ...
        ''',
          variables: [
            ...
            Variable<int>(now.toUtc().microsecondsSinceEpoch),
          ],
        )
        .getSingleOrNull();
    return row != null;
  ...
  Future<void> recordAssetGcQuarantine({
    required String assetId,
    ...
    await _db.customStatement(
      ...
      INSERT INTO asset_gc_quarantine_rows(
        quarantine_path, asset_id, generation, original_path, state, created_at
      ) VALUES (?, ?, ?, ?, 'pending', ?);
    ''',
      [
        quarantinePath,
  ...
  Future<List<AssetGcQuarantineRecord>> listAssetGcQuarantines() async {
    final rows = await _db.customSelect('''
          ...
          ORDER BY created_at, quarantine_path;
        ''').get();
    return [
      for (final row in rows)
        AssetGcQuarantineRecord(
          assetId: row.read<String>('asset_id'),
          generation: row.read<int>('generation'),
          originalPath: row.read<String>('original_path'),
          quarantinePath: row.read<String>('quarantine_path'),
          state: _decodeAssetGcQuarantineState(row.read<String>('state')),
        ),
    ];
  }
  ...
  Future<AssetGcQuarantineRecord?> getAssetGcQuarantine(
    ...
    final row = await _db
        .customSelect(
          ...
          LIMIT 1;
        ''',
          variables: [Variable<String>(quarantinePath)],
        )
        .getSingleOrNull();
    if (row == null) return null;
    return AssetGcQuarantineRecord(
      assetId: row.read<String>('asset_id'),
      generation: row.read<int>('generation'),
      originalPath: row.read<String>('original_path'),
      quarantinePath: row.read<String>('quarantine_path'),
  ...
  Future<AssetGcCompletedDisposition> settleCompletedAssetGcQuarantine({
    ...
    return _db.transaction(() async {
      ...
      await _db.customStatement(
        '''
        DELETE FROM asset_gc_quarantine_rows
        WHERE quarantine_path = ?
          AND asset_id = ?
          AND generation = ?
          AND original_path = ?
  ...
  Future<bool> completeAssetGc({
    ...
    required Set<String> expectedQuarantinePaths,
    required DateTime now,
    ...
    return _db.transaction(() async {
      ...
      for (final row in quarantineRows) {
        ...
        pendingPaths.add(row.read<String>('quarantine_path'));
      }
      if (pendingPaths.length != quarantineRows.length ||
          pendingPaths.length != expectedQuarantinePaths.length ||
          !pendingPaths.containsAll(expectedQuarantinePaths)) {
        return false;
      }
      ...
      await _db.customStatement(
        ...
        UPDATE asset_gc_quarantine_rows
        SET state = 'completed'
        WHERE asset_id = ? AND generation = ? AND state = 'pending';
      ''',
        [assetId, expectedGeneration],
      );
      final completedReceiptCount =
          (await _db.customSelect('SELECT changes() AS changed;').getSingle())
              .read<int>('changed');
      if (completedReceiptCount != expectedQuarantinePaths.length) {
        throw StateError('asset_gc_quarantine_receipt_count_changed');
      }
      await _db.customStatement(
        '''
  ...
  AssetGcQuarantineState _decodeAssetGcQuarantineState(String raw) {
    return switch (raw) {
      'pending' => AssetGcQuarantineState.pending,
      'completed' => AssetGcQuarantineState.completed,
      _ => throw StateError('asset_gc_quarantine_state_invalid'),
    };
  }
  ...
  Future<void> putMessage(ChatMessage message, {int? messageOrder}) async {
    ...
    await _db.transaction(() async {
      await _ensureTurnMetadata(message);
      await _db
  ...
  Future<Conversation> upsertConversationFromSync(Conversation incoming) {
    return _db.transaction(() async {
      final existingRow = await (_db.select(
        _db.conversationRows,
      )..where((row) => row.id.equals(incoming.id))).getSingleOrNull();
      ...
      final synchronized = Conversation(
        ...
        versionSelections: Map<String, int>.from(
          existing?.versionSelections ?? const <String, int>{},
        ),
      ...
      await _db
          .into(_db.conversationRows)
          .insertOnConflictUpdate(_conversationCompanion(synchronized));
      await _replaceMcpServers(synchronized.id, synchronized.mcpServerIds);
      return synchronized;
    });
  }

  Future<Conversation> ensureConversationFromSync({
    required String conversationId,
    required DateTime createdAt,
    ...
    return _db.transaction(() async {
      ...
      await _db
          .into(_db.conversationRows)
          .insert(_conversationCompanion(placeholder), mode: InsertMode.insert);
      return placeholder;
    });
  }

  Future<void> upsertTurnFromSync({
    required String conversationId,
    ...
    return _db.transaction(() async {
      ...
      if (existing != null && existing.conversationId != conversationId) {
        throw StateError('sync_turn_parent_changed');
      }
      await _db
          .into(_db.turnRows)
  ...
  Future<void> _ensureTurnMetadata(ChatMessage message) async {
    final existing = await (_db.select(
      _db.turnRows,
    )..where((row) => row.id.equals(message.turnId))).getSingleOrNull();
    if (existing != null) {
      if (existing.conversationId != message.conversationId) {
        throw StateError('turn_parent_changed');
      }
      return;
    }
    await _db
  ...
  Future<ChatMessage> upsertMessageFromSync(ChatMessage message) {
    return _db.transaction(() async {
      final existing = await (_db.select(
        _db.messageRows,
      )..where((row) => row.id.equals(message.id))).getSingleOrNull();
      if (existing != null &&
          existing.conversationId != message.conversationId) {
        throw StateError('sync_message_parent_changed');
      }
      final order =
          existing?.messageOrder ??
          await _nextMessageOrder(message.conversationId);
      await _ensureTurnMetadata(message);
      await _db
          .into(_db.messageRows)
          .insertOnConflictUpdate(_messageCompanion(message, order));
  ...
  Future<void> deleteTurnMetadata(String turnId) async {
    await (_db.delete(
      _db.turnRows,
    )..where((row) => row.id.equals(turnId))).go();
  }

  Future<void> rebuildSyncedMessageOrder(String conversationId) {
    return _db.transaction(() async {
      final messages = await (_db.select(
        _db.messageRows,
      )..where((row) => row.conversationId.equals(conversationId))).get();
      if (messages.isEmpty) return;
      final persistedTurns = await getTurnCreatedAts(conversationId);
      ...
      for (final message in messages) {
        ...
        final group = (message.turnId, message.groupId ?? message.id);
        groupTimes.update(
          group,
          (value) =>
              message.timestamp.isBefore(value) ? message.timestamp : value,
      ...
      messages.sort((left, right) {
        final leftTurnTime =
            persistedTurns[left.turnId] ?? fallbackTurnTimes[left.turnId]!;
        final rightTurnTime =
            persistedTurns[right.turnId] ?? fallbackTurnTimes[right.turnId]!;
        var compared = leftTurnTime.compareTo(rightTurnTime);
        ...
        if (compared != 0) return compared;
        compared = (left.role == 'user' ? 0 : 1).compareTo(
          right.role == 'user' ? 0 : 1,
        );
        if (compared != 0) return compared;
  ...
  /// Deletes every message group whose first row sits after the anchor
  /// group's first row, returning the updated conversation. Linear-message
  /// semantics: a "new reply" replaces everything that follows the anchor.
  ...
  static void _validateGenerationBeginMessages({
    ...
    if (userMessage != null &&
        (userMessage.conversationId != conversation.id ||
            ...
            userMessage.turnId != assistantMessage.turnId)) {
    ...
    if (assistantMessage.conversationId != conversation.id ||
        ...
        assistantMessage.turnId.trim().isEmpty) {
  ...
  Future<String?> _conversationFingerprint(String schema, String id) async {
    ...
    final messageRows = await _db
        .customSelect(
          ...
          'turn_id, generation_status, version, prompt_tokens, '
    ...
    for (final row in messageRows) {
      ...
      final turnId = data.remove('turn_id')?.toString() ?? '';
      data['turn_ordinal'] = turnOrdinals.putIfAbsent(
        turnId,
        () => turnOrdinals.length,
      );
      messages.add([
  ...
  Future<void> _insertMergedConversation({
    ...
    final sourceMessages = await _db
        .customSelect(
          'SELECT id, group_id, turn_id FROM merge_source.message_rows '
          'WHERE conversation_id = ? ORDER BY message_order, id;',
    ...
    for (final row in sourceMessages) {
      ...
      turnIdMap.putIfAbsent(
        turnId,
        () => remapping
            ? _deterministicMergeId('turn', turnId, targetId)
            : turnId,
      );
    }
    ...
    final sourceTurns = await _db
        .customSelect(
          'SELECT id, created_at FROM merge_source.turn_rows '
          'WHERE conversation_id = ?;',
          variables: [Variable<String>(sourceId)],
        )
        .get();
    for (final sourceTurn in sourceTurns) {
      final sourceTurnId = sourceTurn.read<String>('id');
      await _db.customStatement(
        'INSERT INTO main.turn_rows (id, conversation_id, created_at) '
        'VALUES (?, ?, ?);',
        [
          turnIdMap[sourceTurnId] ?? sourceTurnId,
          targetId,
          sourceTurn.read<int>('created_at'),
        ],
      );
    }
  ...
  Future<void> _writeBackupData({
    ...
    final turns = <String, ({String conversationId, DateTime createdAt})>{};
  ...
  Future<void> _clearChatRows() async {
    ...
    await _db.delete(_db.turnRows).go();
  ...
  Future<bool> hasToolEvents(String messageId) async {
    final legacy = await (_db.select(
      _db.toolEventRows,
    ...
    for (final row in partRows) {
      ...
      if (decoded is! Map<String, dynamic>) {
        throw const FormatException(
          'message_part_rows.tool_call.payload 必须是 JSON 对象',
        );
      }
      result
  ...
  Future<List<ChatMessage>> getStreamingMessages() async {
    final rows = await (_db.select(
      _db.messageRows,
    ...
    return _db.transaction(() async {
      ...
      await (_db.update(
        ...
        const MessageRowsCompanion(
          ...
          generationStatus: Value('interrupted'),
        ),
      );
  ...
  MessageRowsCompanion _messageUpdate(ChatMessage message) {
    return MessageRowsCompanion(
      ...
      generationStatus: Value(message.generationStatus),
      promptTokens: Value(message.promptTokens),
      completionTokens: Value(message.completionTokens),
      cachedTokens: Value(message.cachedTokens),
      durationMs: Value(message.durationMs),
  ...
  Map<String, int> _decodeStringIntMap(String raw) {
    ...
    if (decoded is! Map<String, Object?>) {
      throw const FormatException('chat_database_string_int_map_json');
    }
  ...
  List<Map<String, dynamic>> _decodeToolEvents(String raw) {
    ...
    if (decoded is! List<Object?>) {
      throw const FormatException('chat_database_tool_events_json');
    }
    ...
    return List<Map<String, dynamic>>.unmodifiable(events);
  }

  List<String> _decodeStringList(String raw) {
    final decoded = jsonDecode(raw);
    if (decoded is! List<Object?>) {
      throw const FormatException('chat_database_string_list_json');
    }
    final result = <String>[];
...
final class AssetGcLeaseAttempt {
  const AssetGcLeaseAttempt({required this.acquired, required this.expiresAt});

  final bool acquired;
  final DateTime expiresAt;
}

enum AssetGcQuarantineState { pending, completed }

enum AssetGcCompletedDisposition { restore, delete }

final class AssetGcQuarantineRecord {
  const AssetGcQuarantineRecord({
    required this.assetId,
    required this.generation,
