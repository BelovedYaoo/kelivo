...
/// v2 写入数据后供回滚版本使用的兼容性约定。
///
/// 回滚版本必须保留 v2 读写器并使用相同的 schema 窗口，
/// 绝不能尝试向下迁移，也不能重新把保留的 Hive 文件作为活动写入源。
...
/// Hive -> database v2 迁移使用的本地无秘密推出证据。
///
/// 每次更新都是不可变且带校验和的回执。这样无需原位替换 JSON，
/// 支持人员也能区分完整的最新回执和中断写入，
/// 同时不收集消息正文、ID 或路径。
