...
/// 将经过验证的 v2 恢复载荷复制到应用数据文件系统。
///
/// 候选数据在对应运行工作区中保持不可变，直到启动门禁
/// 提交整个数据包或恢复上一版数据包。
final class RestoreBundleStaging {
  ...
  // 设置中仅包含结构化偏好，不包含聊天行或二进制资源。
  // 复制或解析前限制 JSON 大小，以约束 UTF-8 数据和 DOM 的放大效应。
