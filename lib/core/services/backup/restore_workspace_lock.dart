...
final class RestoreWorkspaceLock {
  ...
  /// 将持锁启动检查器观察到的精确标记转换为持久化切换声明，
  /// 或恢复中断的发布声明。
  ///
  /// 调用方必须在验证同一把锁下的运行和回执链之后，
  /// 从同一个 [synchronized] 动作中调用此方法。
  ...
  /// 将终态运行移出单一活动运行准入区，同时保留其回执、候选和上一版证据。
  ///
  /// 调用方必须持有此工作区锁，并已验证 [runId] 对应的
  /// 已提交或已回滚终态回执。归档标记会一直作为操作先行证据，
  /// 直到运行目录重命名和两个父目录屏障都完成。
  ...
  /// 持久删除能够证明从未发布首份回执的运行。
  ///
  /// 调用方必须持有此工作区锁。只接受精确的暂存及初始回执发布拓扑。
  /// 一旦存在最终回执、上一版数据包、链接、特殊文件系统条目或未知路径，
  /// 就保持工作区不变并失败关闭。
