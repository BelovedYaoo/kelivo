...
final class _WindowsRestoreDurability implements RestoreDurability {
  ...
  static Future<void> _requirePathType(
    ...
    // Windows Application Support 下的文件会继承其用户 ACL。
    // Windows 不存在 chmod 风格的模式变更；下面的直写操作会
    // 保持这条继承的安全边界。
  }
  ...
  Future<void> syncDirectory(
    ...
    // Windows 没有 Apple 风格的独立 F_FULLFSYNC 原语。继续使用
    // 现有 FlushFileBuffers 路径；在受支持文件系统上完成验证前，
    // 其目录行为仍属于平台验收边界。
