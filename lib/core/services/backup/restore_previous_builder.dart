...
/// 在移动任何恢复对象前描述已关闭的活动数据包。
///
/// 文件内容采用流式哈希。所得不可变计划是后续切换阶段的权威；
/// 此构建器绝不修改活动设置、数据库或资源根目录。
final class RestorePreviousBuilder {
  ...
  /// 在首次重命名前，让选中的活动资源根目录持久化。
  ///
  /// 先同步文件，再从最深层到 [root] 同步目录。
  /// 在 Apple 平台上，最终根目录屏障还会为此前所有文件和目录刷新排序。
