...
class ChatController extends ChangeNotifier {
  ...
  Future<bool> openAroundPersistedMessage(ChatMessage message) async {
    ...
    if (visibleIndex >= 0) {
      // 编辑后的版本仍属于同一逻辑时间线槽位。保持当前有界窗口不变，
      // 让列表保留可见锚点，仅重新测量该槽位的尺寸。
  ...
  /// 将一次原子持久化结果作为单次 UI 变更发布到已加载的尾部。
  /// 发送从用户/助手消息对开始，因此若在两条消息之间刷新持久化计数，
