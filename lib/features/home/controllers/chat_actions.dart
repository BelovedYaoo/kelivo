...
class ChatActions {
  ...
  Future<ChatActionResult> regenerateAtMessage({
    ...
    // 重新生成会修改现有逻辑分组。保持已加载窗口围绕该分组，
    // 避免用会话尾部替换远处的阅读位置；长会话中尾部窗口
    // 可能并不包含当前流式版本。
  ...
  Future<void> flushConversationProgress(Conversation? conversation) async {
    ...
    // 即使用户在流式输出中途离开，也要确保转换所有内联 data URL。
