...
class HomePageController extends ChangeNotifier {
  ...
  /// 为新打开的会话提供独立滚动状态，对应 RikkaHub 中
  /// 每个 ChatPage 的 `rememberLazyListState` 生命周期。
  ...
  Future<void> switchConversationAnimated(String id) async {
    ...
    if (!isDesktopPlatform) {
      try {
        await WidgetsBinding.instance.endOfFrame;
        if (currentConversation?.id != id) return;
        // 在新会话仍透明时解析真实末项；首次 maxScrollExtent
        // 可能仍包含懒加载估算值。
  ...
  Future<void> scrollToMessageId(
    ...
    if (useRikkaTransition) {
      try {
        await _messageJumpTransitionController.reverse().orCancel;
      } on TickerCanceled {
        // 页面销毁或新的跳转接管动画时，本次跳转不应继续改动滚动位置。
        return;
      }
    }
    ...
    try {
      ...
      if (useRikkaTransition) {
        try {
          await _messageJumpTransitionController.forward().orCancel;
        } on TickerCanceled {
          // 页面销毁或下一次跳转接管动画时，不再恢复已失效的控制器。
        }
      }
    }
  }
  ...
  // _getViewportBounds 已移除：索引列表会直接暴露可见范围。
