...
abstract interface class BackgroundTaskCancellationRegistration {
  void unregister();
}

final class BackgroundTaskContext {
  BackgroundTaskContext._();

  final Set<void Function()> _cancellationListeners = <void Function()>{};
  bool _cancelled = false;

  bool get isCancelled => _cancelled;

  BackgroundTaskCancellationRegistration registerCancellation(
    void Function() onCancelled,
  ) {
    if (_cancelled) {
      onCancelled();
      return const _NoopBackgroundTaskCancellationRegistration();
    }
    _cancellationListeners.add(onCancelled);
    return _BackgroundTaskCancellationRegistration(() {
...
final class _BackgroundTaskCancellationRegistration
    implements BackgroundTaskCancellationRegistration {
  _BackgroundTaskCancellationRegistration(this._unregister);
...
final class _NoopBackgroundTaskCancellationRegistration
    implements BackgroundTaskCancellationRegistration {
  const _NoopBackgroundTaskCancellationRegistration();
...
///   Workmanager().executeTask((taskName, inputData, context) {
///     switch(taskName) {
///       case "":
...
class Workmanager {
  ...
  static final Map<String, BackgroundTaskContext> _activeTaskContexts =
      <String, BackgroundTaskContext>{};
  static final Set<String> _pendingTaskCancellations = <String>{};
  ...
  /// Android 检查工作是否已入队或运行；iOS 检查 BGTaskScheduler pending request。
...
class _WorkmanagerFlutterApiImpl extends WorkmanagerFlutterApi {
  ...
  Future<bool> executeTask(
    ...
    final context = BackgroundTaskContext._();
    if (Workmanager._pendingTaskCancellations.remove(taskName)) {
      context._cancel();
    }
    Workmanager._activeTaskContexts[taskName] = context;
    try {
      final result = await Workmanager._backgroundTaskHandler?.call(
        taskName,
        convertedInputData,
        context,
      );
      ...
      if (identical(Workmanager._activeTaskContexts[taskName], context)) {
        Workmanager._activeTaskContexts.remove(taskName);
      }
    }
  }

  @override
  Future<void> taskCancelled(String taskName) async {
    final context = Workmanager._activeTaskContexts[taskName];
    if (context == null) {
      Workmanager._pendingTaskCancellations.add(taskName);
      return;
    }
    context._cancel();
  }
