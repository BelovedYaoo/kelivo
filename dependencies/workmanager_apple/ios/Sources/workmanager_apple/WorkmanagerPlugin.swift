...
public class WorkmanagerPlugin: FlutterPluginAppLifeCycleDelegate, FlutterPlugin, WorkmanagerHostApi {
    ...
    private static func handleBGProcessingTask(identifier: String, task: BGProcessingTask) {
        ...
        operation.completionBlock = { [weak operation] in
            ...
            operation?.requestBestEffortCleanup()
        }
    ...
    public static func handlePeriodicTask(identifier: String, task: BGAppRefreshTask, earliestBeginInSeconds: Double?) {
        ...
        do {
            try schedulePeriodicTask(
                taskIdentifier: task.identifier,
                earliestBeginInSeconds: earliestBeginInSeconds ?? (15 * 60)
            )
        } catch {
            logError("Could not reschedule BGAppRefreshTask \(error.localizedDescription)")
            task.setTaskCompleted(success: false)
            return
        }
    ...
    @objc
    @available(iOS 13.0, *)
    private static func schedulePeriodicTask(taskIdentifier identifier: String, earliestBeginInSeconds begin: Double) throws {
    ...
    func registerPeriodicTask(request: PeriodicTaskRequest, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        if #available(iOS 13.0, *) {
            ...
            do {
                try WorkmanagerPlugin.schedulePeriodicTask(
            ...
            completion(.failure(createUnsupportedVersionError(feature: "PeriodicTask")))
        }
    }
