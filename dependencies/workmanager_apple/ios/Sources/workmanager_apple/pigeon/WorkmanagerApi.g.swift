...
protocol WorkmanagerFlutterApiProtocol {
  ...
  func taskCancelled(taskName taskNameArg: String, completion: @escaping (Result<Void, PigeonError>) -> Void)
}
class WorkmanagerFlutterApi: WorkmanagerFlutterApiProtocol {
  ...
  func taskCancelled(taskName taskNameArg: String, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.workmanager_platform_interface.WorkmanagerFlutterApi.taskCancelled\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage([taskNameArg] as [Any?]) { response in
      guard let listResponse = response as? [Any?] else {
        completion(.failure(createConnectionError(withChannelName: channelName)))
        return
      }
